                ...
                if iq_count_leaks.confidence_threshold < iq_count_leaks.result.confidence:
                    ...
                    if leak_rois is not None:
                        for roi in leak_rois:
                            # For each detected leak, send the ROI to the classifier to classify what kind of leak it is
                            ...
                            if iq_classify_leaks.confidence_threshold < iq_classify_leaks.result.confidence:
                                ...
                                cv2.putText(
                                    frame, f"Leak: {iq_classify_leaks.result.label}", (left, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2
                                )
